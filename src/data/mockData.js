// Mock data for services
export const services = [
  {
    id: "1",
    name: "Masaje Anti-stress",
    shortDescription:
      "Relaja tu cuerpo y mente con nuestro masaje anti-stress.",
    description:
      "Nuestro masaje anti-stress está diseñado para aliviar la tensión acumulada y proporcionar una profunda sensación de bienestar. Utilizamos técnicas especiales que combinan movimientos suaves y presión moderada para relajar los músculos y reducir el estrés.",
    category: "corporales",
    price: 5500,
    duration: 60,
    image:
      "https://images.unsplash.com/photo-1600334129128-685c5582fd35?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Reduce el estrés y la ansiedad",
      "Mejora la circulación sanguínea",
      "Alivia dolores musculares",
      "Promueve un sueño reparador",
      "Aumenta los niveles de energía",
    ],
    includes: [
      "Consulta previa",
      "Aromaterapia",
      "Música relajante",
      "Infusión post-tratamiento",
    ],
  },
  {
    id: "2",
    name: "Masaje Descontracturante",
    shortDescription:
      "Elimina contracturas y alivia el dolor muscular crónico.",
    description:
      "El masaje descontracturante es una técnica terapéutica enfocada en eliminar contracturas y nudos musculares. Aplicamos presión profunda en los puntos de tensión para liberar la rigidez y restaurar la movilidad natural de los músculos afectados.",
    category: "corporales",
    price: 6000,
    duration: 50,
    image:
      "https://images.unsplash.com/photo-1519823551278-64ac92734fb1?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Elimina contracturas musculares",
      "Reduce el dolor crónico",
      "Mejora la movilidad articular",
      "Corrige desequilibrios posturales",
      "Previene lesiones",
    ],
    includes: [
      "Evaluación física inicial",
      "Aplicación de aceites terapéuticos",
      "Técnicas de presión profunda",
      "Recomendaciones post-tratamiento",
    ],
  },
  {
    id: "3",
    name: "Masaje con Piedras Calientes",
    shortDescription:
      "Experimenta la relajación profunda con piedras volcánicas calientes.",
    description:
      "Este masaje utiliza piedras volcánicas calientes que se colocan estratégicamente en puntos energéticos del cuerpo. El calor penetra profundamente en los músculos, proporcionando una relajación intensa y aliviando tensiones acumuladas.",
    category: "corporales",
    price: 7000,
    duration: 70,
    image:
      "https://images.unsplash.com/photo-1544161515-4ab6ce6db874?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Relaja profundamente el sistema nervioso",
      "Mejora la circulación sanguínea",
      "Alivia dolores musculares y articulares",
      "Desintoxica el organismo",
      "Equilibra los centros energéticos",
    ],
    includes: [
      "Selección de piedras volcánicas",
      "Aceites esenciales aromáticos",
      "Ambiente con aromaterapia",
      "Té de hierbas post-tratamiento",
    ],
  },
  {
    id: "4",
    name: "Drenaje Linfático Manual",
    shortDescription: "Elimina toxinas y reduce la retención de líquidos.",
    description:
      "El drenaje linfático manual es una técnica suave que estimula el sistema linfático para eliminar toxinas y reducir la retención de líquidos. Los movimientos rítmicos y precisos ayudan a mejorar la circulación linfática y promover la desintoxicación natural del cuerpo.",
    category: "corporales",
    price: 6500,
    duration: 60,
    image:
      "https://images.unsplash.com/photo-1570172619644-dfd03ed5d881?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Reduce la retención de líquidos",
      "Elimina toxinas del organismo",
      "Mejora el sistema inmunológico",
      "Reduce la inflamación",
      "Promueve la regeneración celular",
    ],
    includes: [
      "Evaluación de necesidades específicas",
      "Técnicas de drenaje personalizadas",
      "Recomendaciones para mantener resultados",
      "Infusión desintoxicante",
    ],
  },
  {
    id: "5",
    name: "VelaSlim",
    shortDescription:
      "Tratamiento reductor que combate la celulitis y reafirma la piel.",
    description:
      "VelaSlim es un tratamiento corporal avanzado que combina radiofrecuencia, infrarrojo y vacumterapia para reducir la celulitis, remodelar el contorno corporal y reafirmar la piel. Esta tecnología no invasiva estimula la producción de colágeno y mejora la textura de la piel.",
    category: "corporales",
    price: 8500,
    duration: 45,
    image:
      "https://images.unsplash.com/photo-1616394584738-fc6e612e71b9?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Reduce la celulitis",
      "Reafirma la piel flácida",
      "Mejora el contorno corporal",
      "Estimula la producción de colágeno",
      "Resultados visibles desde las primeras sesiones",
    ],
    includes: [
      "Consulta y evaluación inicial",
      "Medición de zonas a tratar",
      "Aplicación de gel conductor",
      "Seguimiento fotográfico de resultados",
    ],
  },
  {
    id: "6",
    name: "Limpieza Facial Profunda",
    shortDescription:
      "Elimina impurezas y revitaliza tu piel con nuestra limpieza facial completa.",
    description:
      "Nuestra limpieza facial profunda elimina impurezas, puntos negros y células muertas, dejando tu piel completamente renovada. Incluye análisis de piel, exfoliación, extracción, mascarilla personalizada y protección final.",
    category: "faciales",
    price: 4500,
    duration: 60,
    image:
      "https://images.unsplash.com/photo-1570172619644-dfd03ed5d881?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Elimina impurezas y toxinas",
      "Previene el acné y puntos negros",
      "Mejora la textura y luminosidad",
      "Prepara la piel para otros tratamientos",
      "Promueve la regeneración celular",
    ],
    includes: [
      "Análisis de tipo de piel",
      "Desmaquillado y limpieza",
      "Exfoliación personalizada",
      "Extracción de impurezas",
      "Mascarilla según tipo de piel",
      "Hidratación y protección solar",
    ],
  },
  {
    id: "7",
    name: "Hidratación Facial",
    shortDescription:
      "Devuelve la hidratación y vitalidad a tu piel con nuestro tratamiento intensivo.",
    description:
      "Tratamiento intensivo diseñado para pieles deshidratadas y apagadas. Utilizamos productos con ácido hialurónico y otros activos hidratantes que penetran profundamente en la piel, restaurando su equilibrio hídrico y luminosidad natural.",
    category: "faciales",
    price: 5000,
    duration: 45,
    image:
      "https://images.unsplash.com/photo-1512290923902-8a9f81dc236c?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Restaura los niveles óptimos de hidratación",
      "Previene el envejecimiento prematuro",
      "Mejora la elasticidad y firmeza",
      "Calma la piel irritada o sensible",
      "Proporciona un aspecto radiante",
    ],
    includes: [
      "Limpieza suave",
      "Exfoliación enzimática",
      "Sérum de ácido hialurónico",
      "Mascarilla hidratante intensiva",
      "Masaje facial con técnica japonesa",
    ],
  },
  {
    id: "8",
    name: "Tratamiento Anti-Age",
    shortDescription:
      "Combate los signos del envejecimiento con nuestro tratamiento rejuvenecedor.",
    description:
      "Tratamiento facial avanzado que combate eficazmente los signos del envejecimiento. Utilizamos péptidos, vitamina C y otros activos anti-edad que estimulan la producción de colágeno, reducen líneas de expresión y mejoran la firmeza de la piel.",
    category: "faciales",
    price: 7500,
    duration: 75,
    image:
      "https://images.unsplash.com/photo-1560750588-73207b1ef5b8?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Reduce líneas de expresión y arrugas",
      "Aumenta la producción de colágeno y elastina",
      "Mejora la firmeza y elasticidad",
      "Unifica el tono y textura de la piel",
      "Proporciona un efecto lifting natural",
    ],
    includes: [
      "Diagnóstico facial completo",
      "Limpieza profunda",
      "Peeling enzimático",
      "Sérum con péptidos y vitamina C",
      "Masaje lifting con técnica Kobido",
      "Mascarilla reafirmante",
      "Protección solar específica",
    ],
  },
  {
    id: "9",
    name: "Punta de Diamante",
    shortDescription:
      "Microdermoabrasión con punta de diamante para una piel renovada y luminosa.",
    description:
      "La microdermoabrasión con punta de diamante es un tratamiento de exfoliación mecánica que elimina suavemente las capas superficiales de la piel. Ideal para tratar manchas, cicatrices leves, líneas finas y piel apagada, dejando el rostro con una apariencia más joven y radiante.",
    category: "faciales",
    price: 6500,
    duration: 45,
    image:
      "https://images.unsplash.com/photo-1487412947147-5cebf100ffc2?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Elimina células muertas e impurezas",
      "Reduce manchas y cicatrices leves",
      "Minimiza poros dilatados",
      "Estimula la producción de colágeno",
      "Mejora la absorción de productos cosméticos",
    ],
    includes: [
      "Limpieza facial previa",
      "Sesión de microdermoabrasión personalizada",
      "Mascarilla calmante post-tratamiento",
      "Hidratación profunda",
      "Protección solar",
    ],
  },
  {
    id: "10",
    name: "Radiofrecuencia Facial",
    shortDescription:
      "Reafirma y tensa la piel con nuestra avanzada radiofrecuencia facial.",
    description:
      "La radiofrecuencia facial es un tratamiento no invasivo que utiliza energía térmica para estimular la producción de colágeno y elastina. El calor controlado penetra en las capas profundas de la piel, logrando un efecto lifting y mejorando visiblemente la firmeza facial.",
    category: "faciales",
    price: 7000,
    duration: 50,
    image:
      "https://images.unsplash.com/photo-1598452963314-b09f397a5c48?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Efecto lifting sin cirugía",
      "Reafirma óvalo facial y cuello",
      "Reduce la flacidez cutánea",
      "Mejora la definición del contorno facial",
      "Resultados progresivos y duraderos",
    ],
    includes: [
      "Limpieza facial previa",
      "Aplicación de gel conductor",
      "Sesión de radiofrecuencia personalizada",
      "Mascarilla reafirmante",
      "Sérum tensor",
      "Protección solar",
    ],
  },
  {
    id: "11",
    name: "Criofrecuencia Facial",
    shortDescription:
      "Combina el poder del frío y la radiofrecuencia para resultados espectaculares.",
    description:
      "La criofrecuencia facial es un tratamiento innovador que alterna radiofrecuencia (calor) y crioterapia (frío) para maximizar los beneficios de ambas tecnologías. Esta combinación estimula intensamente la producción de colágeno, reduce la inflamación y proporciona un efecto lifting inmediato.",
    category: "faciales",
    price: 7000,
    duration: 50,
    image:
      "https://images.unsplash.com/photo-1571942676516-bcab84649e44?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Efecto lifting inmediato",
      "Reduce la inflamación y rojeces",
      "Cierra poros dilatados",
      "Mejora la oxigenación celular",
      "Potencia la regeneración de la piel",
    ],
    includes: [
      "Diagnóstico facial",
      "Limpieza profunda",
      "Sesión de radiofrecuencia",
      "Aplicación de crioterapia",
      "Mascarilla LED",
      "Sérum específico según necesidades",
      "Protección final",
    ],
  },
  {
    id: "12",
    name: "Manicura Spa",
    shortDescription:
      "Cuida y embellece tus manos con nuestra manicura completa.",
    description:
      "Nuestra manicura spa es un tratamiento completo que incluye limado, tratamiento de cutículas, exfoliación, masaje hidratante y esmaltado. Utilizamos productos de alta calidad para dejar tus manos suaves, hidratadas y perfectamente arregladas.",
    category: "belleza",
    price: 2500,
    duration: 40,
    image:
      "https://images.unsplash.com/photo-1604654894610-df63bc536371?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Mantiene las uñas sanas y fuertes",
      "Hidrata profundamente la piel",
      "Previene el envejecimiento de las manos",
      "Mejora la circulación",
      "Proporciona un aspecto cuidado y elegante",
    ],
    includes: [
      "Remoción de esmalte anterior",
      "Limado y conformación de uñas",
      "Tratamiento de cutículas",
      "Exfoliación suave",
      "Masaje con crema hidratante",
      "Esmaltado con color a elección",
    ],
  },
  {
    id: "13",
    name: "Pedicura Spa",
    shortDescription:
      "Renueva y embellece tus pies con nuestro tratamiento completo.",
    description:
      "Nuestra pedicura spa es un tratamiento completo que incluye baño de pies, eliminación de durezas, tratamiento de cutículas, exfoliación, masaje y esmaltado. Ideal para mantener tus pies saludables, suaves y estéticamente perfectos.",
    category: "belleza",
    price: 3000,
    duration: 50,
    image:
      "https://images.unsplash.com/photo-1519415510236-718bdfcd89c8?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Elimina durezas y callosidades",
      "Previene problemas podológicos",
      "Mejora la circulación",
      "Relaja y reduce la fatiga",
      "Hidrata profundamente la piel",
    ],
    includes: [
      "Baño de pies con sales minerales",
      "Eliminación de durezas",
      "Limado y conformación de uñas",
      "Tratamiento de cutículas",
      "Exfoliación con scrub natural",
      "Masaje con crema hidratante",
      "Esmaltado con color a elección",
    ],
  },
  {
    id: "14",
    name: "Depilación Definitiva",
    shortDescription:
      "Elimina el vello no deseado de forma permanente con nuestra tecnología láser.",
    description:
      "Nuestro sistema de depilación definitiva utiliza la más avanzada tecnología láser para eliminar el vello no deseado de forma permanente. El tratamiento es seguro, efectivo y personalizado según tu tipo de piel y vello, garantizando resultados óptimos con el mínimo de sesiones.",
    category: "belleza",
    price: 4000,
    duration: 30,
    image:
      "https://images.unsplash.com/photo-1560750588-73207b1ef5b8?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Eliminación permanente del vello",
      "Tratamiento rápido e indoloro",
      "Previene la foliculitis",
      "Piel más suave y tersa",
      "Ahorro de tiempo a largo plazo",
    ],
    includes: [
      "Consulta y evaluación previa",
      "Preparación de la zona a tratar",
      "Aplicación de láser personalizada",
      "Gel calmante post-tratamiento",
      "Recomendaciones de cuidado",
    ],
  },
  {
    id: "15",
    name: "Maquillaje Profesional",
    shortDescription:
      "Luce radiante en cualquier ocasión con nuestro servicio de maquillaje profesional.",
    description:
      "Nuestro servicio de maquillaje profesional está diseñado para realzar tu belleza natural y adaptarse a cualquier ocasión. Utilizamos productos de alta gama y técnicas avanzadas para crear un look personalizado que refleje tu estilo y personalidad.",
    category: "belleza",
    price: 4500,
    duration: 60,
    image:
      "https://images.unsplash.com/photo-1522335789203-aabd1fc54bc9?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Maquillaje personalizado según tu estilo",
      "Productos de alta gama hipoalergénicos",
      "Técnicas que realzan tus rasgos naturales",
      "Larga duración garantizada",
      "Asesoramiento de imagen incluido",
    ],
    includes: [
      "Preparación de la piel",
      "Base y corrección",
      "Maquillaje de ojos personalizado",
      "Definición de cejas",
      "Labios y mejillas",
      "Fijación para larga duración",
      "Consejos para retoques",
    ],
  },
  {
    id: "16",
    name: "Hidromasajes",
    shortDescription:
      "Relájate y disfruta de los beneficios terapéuticos de nuestros hidromasajes.",
    description:
      "Nuestros hidromasajes combinan los beneficios terapéuticos del agua caliente con potentes jets de masaje para proporcionar una experiencia de relajación completa. Ideal para aliviar tensiones musculares, mejorar la circulación y reducir el estrés.",
    category: "grupales",
    price: 3500,
    duration: 45,
    image:
      "https://images.unsplash.com/photo-1531825552325-9b2cb8b6a88b?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Relaja la musculatura",
      "Mejora la circulación sanguínea",
      "Reduce el estrés y la ansiedad",
      "Alivia dolores articulares",
      "Favorece la eliminación de toxinas",
    ],
    includes: [
      "Sesión de hidromasaje personalizada",
      "Sales minerales terapéuticas",
      "Aromaterapia",
      "Toallas y batas",
      "Infusión relajante post-sesión",
    ],
  },
  {
    id: "17",
    name: "Sauna Finlandés",
    shortDescription:
      "Experimenta los beneficios desintoxicantes del auténtico sauna finlandés.",
    description:
      "Nuestro sauna finlandés tradicional utiliza calor seco a alta temperatura para proporcionar una profunda desintoxicación. El calor dilata los poros, elimina toxinas a través del sudor y proporciona una sensación de bienestar general y relajación profunda.",
    category: "grupales",
    price: 3000,
    duration: 30,
    image:
      "https://images.unsplash.com/photo-1554629947-334ff61d85dc?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Elimina toxinas a través del sudor",
      "Mejora la circulación sanguínea",
      "Fortalece el sistema inmunológico",
      "Relaja la musculatura",
      "Mejora la calidad del sueño",
    ],
    includes: [
      "Sesión de sauna finlandés",
      "Esencias aromáticas de eucalipto",
      "Toallas y batas",
      "Ducha refrescante",
      "Hidratación post-sauna",
    ],
  },
  {
    id: "18",
    name: "Circuito de Aguas",
    shortDescription:
      "Disfruta de nuestro completo circuito de aguas para una experiencia de bienestar total.",
    description:
      "Nuestro circuito de aguas es una experiencia completa que combina diferentes instalaciones acuáticas terapéuticas. Incluye piscina climatizada, jacuzzi, sauna, baño turco y duchas de sensaciones, diseñadas para proporcionar beneficios específicos para el cuerpo y la mente.",
    category: "grupales",
    price: 5000,
    duration: 90,
    image:
      "https://images.unsplash.com/photo-1519823551278-64ac92734fb1?w=800&auto=format&fit=crop&q=60",
    benefits: [
      "Experiencia completa de hidroterapia",
      "Mejora la circulación y el sistema linfático",
      "Reduce el estrés y la tensión muscular",
      "Desintoxica el organismo",
      "Proporciona bienestar general",
    ],
    includes: [
      "Acceso completo al circuito de aguas",
      "Guía de uso recomendado",
      "Toallas y batas",
      "Productos de higiene personal",
      "Infusión o zumo natural",
    ],
  },
];

// Mock data for team members
export const teamMembers = [
  {
    id: "1",
    name: "Dr. María Felicidad",
    position: "Directora & Masajista Terapéutica",
    bio: "Con más de 15 años de experiencia en terapias corporales, María Felicidad fundó Sentirse Bien Spa con la visión de crear un espacio donde el bienestar integral sea la prioridad. Especializada en masajes terapéuticos y técnicas de relajación profunda.",
    image:
      "https://i.pinimg.com/736x/ae/f3/ae/aef3ae50d041b2ebd0a0e3a3b142e6b3.jpg",
    specialties: [
      "Masajes Terapéuticos",
      "Técnicas de Relajación",
      "Aromaterapia",
    ],
  },
  {
    id: "2",
    name: "Carlos Gómez",
    position: "Fisioterapeuta",
    bio: "Fisioterapeuta graduado con honores y especializado en rehabilitación deportiva. Carlos combina técnicas tradicionales con los últimos avances en fisioterapia para proporcionar tratamientos efectivos y personalizados para cada cliente.",
    image:
      "https://i.pinimg.com/736x/63/4d/a8/634da882bde9f66ddd3595ab3d51b1f0.jpg",
    specialties: [
      "Rehabilitación Deportiva",
      "Masaje Descontracturante",
      "Terapia Manual",
    ],
  },
  {
    id: "3",
    name: "Laura Fernández",
    position: "Esteticista & Cosmetóloga",
    bio: "Con formación internacional en cosmética avanzada, Laura es experta en tratamientos faciales y corporales. Su enfoque holístico y atención al detalle garantizan resultados excepcionales en cada tratamiento.",
    image:
      "https://i.pinimg.com/736x/d7/fb/33/d7fb332b6662c543fab7912e2e9f9836.jpg",
    specialties: [
      "Tratamientos Faciales",
      "Cosmética Avanzada",
      "Rejuvenecimiento",
    ],
  },
  {
    id: "4",
    name: "Javier Martínez",
    position: "Terapeuta Holístico",
    bio: "Javier integra diversas técnicas de terapia holística para proporcionar un enfoque completo del bienestar. Especializado en reflexología, reiki y técnicas de meditación, ayuda a sus clientes a encontrar equilibrio físico y emocional.",
    image:
      "https://i.pinimg.com/736x/92/d2/20/92d220a3f15e85c4e5931fc55a5fb64c.jpg",
    specialties: ["Reflexología", "Reiki", "Meditación Guiada"],
  },
  {
    id: "5",
    name: "Sofía López",
    position: "Nutricionista",
    bio: "Licenciada en Nutrición con especialización en nutrición holística. Sofía cree en el poder de la alimentación como base del bienestar y trabaja con cada cliente para desarrollar planes nutricionales personalizados que complementen sus tratamientos en el spa.",
    image:
      "https://i.pinimg.com/736x/64/62/c3/6462c33a59a856a696435f33a9312854.jpg",
    specialties: [
      "Nutrición Holística",
      "Planes Alimentarios",
      "Detox Natural",
    ],
  },
  {
    id: "6",
    name: "Laura Vallejos",
    position: "Manicurista & Cosmiatra",
    bio: "Con varios años de experiencia en el mundo de los tratamientos de belleza, Laura se unió a Sentirse Bien Spa con la misión de brindar a cada cliente una experiencia de cuidado personal única. Especializada en manicura, pedicura, y tratamientos de belleza y relajación, Laura se enfoca en ofrecer servicios de alta calidad con atención al detalle y bienestar integral.",
    image:
      "https://i.pinimg.com/736x/c6/60/7f/c6607f527430f5f840b77d7b198a3fbf.jpg",
    specialties: [
      "Manicura y Pedicura",
      "Tratamientos de Belleza para manos y pies",
      "Técnicas de Rejuvenecimiento Facial",
    ],
  },
];

// Mock data for testimonials
export const testimonials = [
  {
    id: "1",
    name: "Ana García",
    text: "Visito Sentirse Bien Spa regularmente para sus masajes anti-stress y siempre salgo renovada. El ambiente es tranquilo y el personal extremadamente profesional. ¡Una experiencia que recomiendo totalmente!",
    rating: 5,
    service: "Masaje Anti-stress",
    image: "/images/testimonials/ana.jpg",
  },
  {
    id: "2",
    name: "Roberto Sánchez",
    text: "Después de una lesión deportiva, los tratamientos de fisioterapia con Carlos han sido fundamentales para mi recuperación. Su conocimiento y profesionalismo son excepcionales. El spa en general tiene instalaciones de primera calidad.",
    rating: 5,
    service: "Fisioterapia Deportiva",
    image: "/images/testimonials/roberto.jpg",
  },
  {
    id: "3",
    name: "Elena Martínez",
    text: "Los tratamientos faciales con Laura han transformado mi piel. Después de probar muchos lugares, finalmente encontré un spa donde realmente entienden mis necesidades y ofrecen soluciones efectivas. ¡Mi piel nunca ha lucido mejor!",
    rating: 5,
    service: "Tratamiento Anti-Age",
    image: "/images/testimonials/elena.jpg",
  },
  {
    id: "4",
    name: "Miguel Torres",
    text: "El circuito de aguas es una experiencia increíble. Las instalaciones están impecables y el personal siempre atento a cada detalle. Es mi lugar favorito para desconectar del estrés diario.",
    rating: 4,
    service: "Circuito de Aguas",
    image: "/images/testimonials/miguel.jpg",
  },
  {
    id: "5",
    name: "Carolina Díaz",
    text: "La consulta nutricional con Sofía complementó perfectamente mis tratamientos corporales. Su enfoque integral y consejos prácticos me han ayudado a mantener los resultados a largo plazo. Todo el equipo del spa es excepcional.",
    rating: 5,
    service: "Consulta Nutricional",
    image: "/images/testimonials/carolina.jpg",
  },
  {
    id: "6",
    name: "Carolina Díaz",
    text: "La consulta nutricional con Sofía complementó perfectamente mis tratamientos corporales. Su enfoque integral y consejos prácticos me han ayudado a mantener los resultados a largo plazo. Todo el equipo del spa es excepcional.",
    rating: 5,
    service: "Consulta Nutricional",
    image: "/images/testimonials/carolina.jpg",
  },
];

// Mock data for spa information
export const spaInfo = {
  name: "Sentirse Bien Spa",
  slogan: "Tu oasis de bienestar en el corazón de la ciudad",
  address: "Av. Libertad 1234, Resistencia, Chaco",
  phone: "+54 9 3624 123456",
  email: "info@sentirsebien.com",
  socialMedia: {
    facebook: "https://facebook.com/sentirsebien",
    instagram: "https://instagram.com/sentirsebien_spa",
    twitter: "https://twitter.com/sentirsebien_spa",
  },
  hours: {
    monday: "9:00 - 20:00",
    tuesday: "9:00 - 20:00",
    wednesday: "9:00 - 20:00",
    thursday: "9:00 - 20:00",
    friday: "9:00 - 20:00",
    saturday: "10:00 - 18:00",
    sunday: "Cerrado",
  },
  about: {
    history:
      "Fundado en 2010 por la Doctora Felicidad, Sentirse Bien Spa nació con la misión de ofrecer un enfoque integral del bienestar en un ambiente de lujo accesible. Desde entonces, nos hemos convertido en el referente de tratamientos corporales y faciales en la región, combinando técnicas tradicionales con las últimas innovaciones en estética y bienestar.",
    mission:
      "Nuestra misión es proporcionar experiencias de bienestar transformadoras que equilibren cuerpo, mente y espíritu, en un ambiente de serenidad y lujo accesible.",
    vision:
      "Ser reconocidos como el spa líder en bienestar integral, donde cada cliente encuentre soluciones personalizadas para sus necesidades específicas, con los más altos estándares de calidad y servicio.",
  },
  features: [
    "Instalaciones de lujo con ambiente relajante",
    "Profesionales altamente calificados y certificados",
    "Productos premium y tecnología de vanguardia",
    "Tratamientos personalizados según necesidades específicas",
    "Enfoque holístico del bienestar",
  ],
};
